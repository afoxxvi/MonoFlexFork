...
class _predictor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        elif use_norm == 'AN': norm_func = AttnBatchNorm2d
        ...
        self.use_attn_norm = cfg.MODEL.ATTN_NORM
        ...
        self.class_head = self._build_head(in_channels, classes)
        self.class_head[-1].bias.data.fill_(- np.log(1 / cfg.MODEL.HEAD.INIT_P - 1))
        num_kpt = cfg.MODEL.HEAD.NUM_KPT
        self.kpt_hm_head = self._build_head(in_channels, num_kpt)
        self.kpt_hm_head[-1].bias.data.fill_(- np.log(1 / cfg.MODEL.HEAD.INIT_P - 1))
        ...
        for idx, regress_head_key in enumerate(self.regression_head_cfg):
            feat_layer = self._build_head_feet(in_channels)
    ...
    def _build_head(self, in_channels, out_channels):
        if self.use_inplace_abn:
            return nn.Sequential(
                ...
                nn.Conv2d(self.head_conv, out_channels, kernel_size=1, padding=1 // 2, bias=True),
            )
        elif self.use_attn_norm:
            return nn.Sequential(
                nn.Conv2d(in_channels, self.head_conv, kernel_size=3, padding=1, bias=False),
                AttnBatchNorm2d(self.head_conv, 10, momentum=0.03, eps=0.001),
    ...
    def _build_head_feet(self, in_channels):
        if self.use_inplace_abn:
            return nn.Sequential(
                nn.Conv2d(in_channels, self.head_conv, kernel_size=3, padding=1, bias=False),
                InPlaceABN(self.head_conv, momentum=self.bn_momentum, activation=self.abn_activision)
    ...
    def forward(self, features, targets):
        ...
        feature_hm_kpt = self.kpt_hm_head[:-1](features)
        output_hm_kpt = self.kpt_hm_head[-1](feature_hm_kpt)
